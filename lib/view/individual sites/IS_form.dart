...
import 'individual_sites_view.dart';
                ...
                children: [
                  ...
                  Obx(() {
                    ...
                    return CustomDropdown1(
                      ...
                      width: media1,
                      height: 80,
                      enabled: isEnabled, // ✅ enable only when city is selected
                      ontap: () {
                        if (!isEnabled) return;
                        ...
                        if (selectedArea.value.isNotEmpty) {
                          ...
                          if (matchedArea.areaId != null) {
                            ...
                            print("Selected Area ID: $area_ID");
                          } else {
                            area_ID = null;
                        ...
                        ontap: () {
                          if (selectedSoftAccountHolders.value.isNotEmpty) {
                            ...
                            if (matchedRetailer.retailerId != null) {
                              retailer_id = matchedRetailer.retailerId as String;
                  ...
                  CustomButton1(
                    ...
                    onPressed: () async {
                      if (selectedVia.value.isEmpty) {
                        ...
                        CustomToast('Please Add Customer Contact Number',
                          ...
                          !customerContactController.value.text
                              .startsWith('03')) {
                        CustomToast(
                            'Please ENTER valid Customer Contact Number (e.g. 03XXXXXXXXX)',
                            context: context);
                      } else if (customerNameAddressController
                          .value.text.isEmpty) {
                        CustomToast('Please Add Customer Name and Address',
                            context: context);
                      ...
                      } else if (isReferralSelected.value &&
                          selectedReferralArea.value.isEmpty) {
                        CustomToast('Please Select Referral Area',
                            context: context);
                      } else if (isReferralSelected.value &&
                          selectedSalesOfficer.value.isEmpty) {
                        CustomToast('Please Select Sales Officer',
                            context: context);
                      } else if (selectedVia.value == 'RETAILER' &&
                          selectedSoftAccountHolders.value.isEmpty) {
                        CustomToast('Please Select Retailer', context: context);
                      } else if (selectedSecondPersonType.value.isEmpty) {
                        CustomToast('Please Select Second Person Type',
                            context: context);
                      } else if (secoundPersonNameController
                          .value.text.isEmpty) {
                        CustomToast('Please Enter Second Person Name',
                            context: context);
                        ...
                        CustomToast('Please Select Customer House Size',
                            context: context);
                      } else if (selectedPlannedVisitDate.value == null) {
                        CustomToast('Please Select Planned Visit Date',
                            context: context);
                      } else if (selectedVia.value == 'MARKETING ACTIVITIES' &&
                          selectedMktDate.value == null) {
                        CustomToast('Please Select MKT Date', context: context);
                      } else if (ExpectedKgsController.value.text.isEmpty) {
                        CustomToast('Please Enter Expected Kgs',
                            context: context);
                      } else if (selectedVia.value == 'HUNTING' &&
                          selectedTypeHunting.value.isEmpty) {
                        CustomToast('Please Select Hunting Type',
                            context: context);
                      } else if (selectedVia.value == 'PAINTER' &&
